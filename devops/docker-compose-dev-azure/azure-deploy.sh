...
resourcename="lawson"  # lower case, only
resourcegroup="lawson_rg"
...
# git clone https://github.com/apilogicserver/lawson.git
# cd lawson
...
az group create --name lawson_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group lawson_rg --sku S1 --is-linux
az container create --resource-group lawson_rg --name lawson --image apilogicserver/lawson:latest --dns-name-label lawson --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//lawson.westus.azurecontainer.io:5656'
